...
class my_Layernorm(nn.Cell):
    ...
    def forward(self, x):
        ...
        bias = ops.ReduceMean(keep_dims=True)(x_hat, 1).unsqueeze(1).repeat(1, x.shape[1], 1)
        return x_hat - bias
...
class series_decomp_multi(nn.Cell):
    ...
    def construct(self, x):
        ...
        moving_mean = ops.ReduceSum(keep_dims=True)(moving_mean * ops.Softmax(axis=-1)(self.layer(x.unsqueeze(-1))), 1)
        res = x - moving_mean
        return res, moving_mean
