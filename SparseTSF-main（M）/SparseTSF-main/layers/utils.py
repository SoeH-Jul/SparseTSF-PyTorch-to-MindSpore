...
class UnitGaussianNormalizer(object):
    def __init__(self, x, eps=0.00001):
        ...
        self.mean = ops.Mean(keep_dims=True)(x)
        self.std = ops.StandardDeviation(keep_dims=True)(x)
        self.eps = eps
...
class GaussianNormalizer(object):
    def __init__(self, x, eps=0.00001):
        ...
        self.std = ops.StandardDeviation()(x)
        self.eps = eps
...
class RangeNormalizer(object):
    def __init__(self, x, low=0.0, high=1.0):
        ...
        mymin = ops.ReduceMin(x, 0).view(-1)
        mymax = ops.ReduceMax(x, 0).view(-1)
...
class LpLoss(object):
    ...
    def rel(self, x, y):
        ...
        if self.reduction:
            if self.size_average:
                return ops.ReduceMean()(diff_norms / y_norms)
            else:
                return ops.ReduceSum()(diff_norms / y_norms)
