...
class PositionalEmbedding(nn.Cell):
    def __init__(self, d_model, max_len=5000):
        ...
        pe = ops.Zeros()(max_len, d_model).astype(mindspore.float32)
        pe.require_grad = False

        position = ops.Range(0, max_len).reshape(-1, 1).astype(mindspore.float32)
        div_term = (ops.Range(0, d_model, 2).astype(mindspore.float32) * -(math.log(10000.0) / d_model)).exp()
...
class FixedEmbedding(nn.Cell):
    def __init__(self, c_in, d_model):
        ...
        w = ops.Zeros()(c_in, d_model).astype(mindspore.float32)
        w.require_grad = False

        position = ops.Range(0, c_in).reshape(-1, 1).astype(mindspore.float32)
        div_term = (ops.Range(0, d_model, 2).astype(mindspore.float32) * -(math.log(10000.0) / d_model)).exp()
...
class TimeFeatureEmbedding(nn.Cell):
    def __init__(self, d_model, embed_type='timeF', freq='h'):
        ...
        d_inp = freq_map[freq]
        self.embed = nn.Dense(d_inp, d_model, bias=False)
