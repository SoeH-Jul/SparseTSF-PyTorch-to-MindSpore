...
class PatchTST_backbone(nn.Cell):
    ...
    def construct(self, z: Tensor) -> Tensor:  # z: [bs x nvars x seq_len]
        # norm
        if self.revin:
            z = z.transpose(0, 2, 1)
        ...
        z = ops.Unfold(1, self.patch_len, self.stride)(z)  # z: [bs x nvars x patch_num x patch_len]
        z = z.transpose(0, 1, 3, 2)  # z: [bs x nvars x patch_len x patch_num]
    ...
    def create_pretrain_head(self, head_nf, vars, dropout):
        return nn.SequentialCell([nn.Dropout(dropout),
...
class Flatten_Head(nn.Cell):
    ...
    def construct(self, x):  # x: [bs x nvars x d_model x patch_num]
        if self.individual:
            x_out = []
            for i in range(self.n_vars):
                ...
                z = self.dropouts[i](z)
                x_out.append(z)
            x = ops.Stack(axis=1)(x_out)  # x: [bs x nvars x target_window]
        else:
            x = self.flatten(x)
            x = self.linear(x)
            x = self.dropout(x)
...
class TSTiEncoder(nn.Cell):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # Input encoding
        q_len = patch_num
        self.W_P = nn.Dense(patch_len, d_model)  # Eq 1: projection of feature vectors onto a d-dim vector space
        self.seq_len = q_len
    ...
    def construct(self, x: Tensor) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        u = ops.Reshape()(x, (x.shape[0] * x.shape[1], x.shape[2], x.shape[3]))  # u: [bs * nvars x patch_num x d_model]
        u = self.dropout(u + self.W_pos)  # u: [bs * nvars x patch_num x d_model]

        # Encoder
        z = self.encoder(u)  # z: [bs * nvars x patch_num x d_model]
        z = ops.Reshape()(z, (-1, n_vars, z.shape[-2], z.shape[-1]))  # z: [bs x nvars x patch_num x d_model]
...
class TSTEncoderLayer(nn.Cell):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
        ...
        self.ff = nn.SequentialCell([nn.Dense(d_model, d_ff, bias=bias),
    ...
    def construct(self, src: Tensor, prev: Optional[Tensor] = None, key_padding_mask: Optional[Tensor] = None,
...
class _MultiheadAttention(nn.Cell):
    def __init__(self, d_model, n_heads, d_k=None, d_v=None, res_attention=False, attn_dropout=0., proj_dropout=0.,
        ...
        self.W_Q = nn.Dense(d_model, d_k * n_heads, bias=qkv_bias)
        self.W_K = nn.Dense(d_model, d_k * n_heads, bias=qkv_bias)
        self.W_V = nn.Dense(d_model, d_v * n_heads, bias=qkv_bias)
        ...
        # Poject output
        self.to_out = nn.SequentialCell(nn.Dense(n_heads * d_v, d_model), nn.Dropout(proj_dropout))

    def construct(self, Q: Tensor, K: Optional[Tensor] = None, V: Optional[Tensor] = None,
        ...
        output = output.transpose(1, 2).reshape(bs, -1, self.n_heads * self.d_v)  # output: [bs x q_len x n_heads * d_v]
...
class _ScaledDotProductAttention(nn.Cell):
    ...
    def construct(self, q: Tensor, k: Tensor, v: Tensor, prev: Optional[Tensor] = None,
        ...
        attn_scores = ops.Matmul(q, k) * self.scale  # attn_scores : [bs x n_heads x max_q_len x q_len]
        ...
        if key_padding_mask is not None:  # mask with shape [bs x q_len] (only when max_w_len == q_len)
            attn_scores = ops.MaskedFill(attn_scores, key_padding_mask.unsqueeze(1).unsqueeze(2), -np.inf)
        # normalize the attention weights
        attn_weights = ops.Softmax(axis=-1)(attn_scores)  # attn_weights   : [bs x n_heads x max_q_len x q_len]
        attn_weights = self.attn_dropout(attn_weights)

        # compute the new values given the attention weights
        output = ops.Matmul(attn_weights, v)  # output: [bs x n_heads x max_q_len x d_v]
